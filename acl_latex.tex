...
\begin{abstract}
As agentic LLM systems move from prototypes to deployment, evaluating them has become more important and more difficult. Evaluation goals are often left implicit. Without a clear account of what a system is expected to do, how it can fail, and which failures matter, metric choices become difficult to justify, interpret, or validate. We present Litmus, a zero-label system that designs evaluation and monitoring metrics for AI pipelines by eliciting evaluation intent from source code and targeted interrogation. Litmus first identifies what must be measured and why, then converts those answers into constraints for a justified, per-stage metric portfolio. We evaluate Litmus on three real, code-defined pipelines against AutoMetrics and three DynamicRubric baselines. Litmus achieves broader concern coverage on account grouping at matched size, spans more stages, produces a near-zero-redundancy portfolio, and, under a zero-label design-time constraint, ranks first in validity against per-row quality labels on all three pipelines, decisively on scientific QA ($\rho{=}0.72$ vs.\ $0.47$ for the strongest baseline on the stitched single-vector score), with overlapping intervals on the other two. Results support a shift from automatic metric implementation to automatic metric specification: before asking which metric to compute, evaluation systems should ask what must be measured and why.
\end{abstract}
...
Evaluation in deployed AI systems serves a broader role than benchmark comparison: it must support debugging, regression testing, monitoring, auditability, and operational decision-making. Prior work on production machine learning emphasizes that deployed ML systems accumulate hidden technical debt through data dependencies, configuration assumptions, feedback loops, and boundary failures \citep{NIPS2015_86df7dcf}. \citet{8258038} argue for systematic tests that go beyond aggregate model quality, including data validation, infrastructure checks, and monitoring; \citet{8804457} similarly show that engineering AI-enabled systems requires connecting model behavior to data, code, and deployment context. Documentation and auditing frameworks make a related point: intended use, limitations, assumptions, and evaluation evidence should be explicit in deployed AI systems \citep{10.1145/3287560.3287596,10.1145/3458723,10.1145/3351095.3372873}. Litmus follows this production-oriented view: it treats metric design as a system-level task in which metrics should be grounded in components, failure surfaces, and monitoring needs rather than only in final outputs.

A growing body of work scrutinizes how metrics are used and reported. \citet{schmidtova-etal-2024-automatic-metrics} survey current NLG evaluation practice and find pervasive missing rationales, missing implementation details, and missing correlations with human judgement; \citet{zhou-etal-2022-deconstructing} trace the ``kitchen-sink'' tendency to unclear evaluation goals. Validity critiques of overlap metrics \citep{reiter-2018-structured,papineni-etal-2002-bleu,lin-2004-rouge} motivate the move toward LLM-as-judge evaluation \citep{liu-etal-2023-g,fu-etal-2024-gptscore}. However, LLM-as-judge methods do not remove the need for specification: a judge prompt still encodes assumptions about what matters, what evidence should be considered, and how scores should be interpreted. Prior work also shows that LLM judges may exhibit systematic biases and model-family alignment effects \citep{zheng2023judging}. Litmus targets the specification gap these works identify: it produces metrics whose rationale is a first-class artifact.

Recent benchmarks measure general agent capability across reasoning-and-acting, tool use, web navigation, and software-engineering tasks \citep{yao2023react,qin2024toolllm,liu2024agentbench,zhou2024webarena,jimenez2024swebench}. Deployed industry pipelines, however, contain domain-specific rules, retrieval layers, routing logic, fallbacks, confidence thresholds, and compliance constraints. Litmus addresses a complementary problem: designing metrics for a particular code-defined pipeline by identifying what should be measured, at which stage, and why.

The closest conceptual neighbour to our framing is clarification-question generation. \citet{rao-daume-iii-2018-learning} formalize ``a good question is one whose expected answer is most useful'' via the expected value of perfect information, \citet{rao-daume-iii-2019-answer} generate rather than merely rank such questions, and \citet{majumder-etal-2021-ask} identify information ``essential to accomplish an underlying goal but currently missing from the context.'' Litmus adopts this usefulness-of-answer stance: every question it surfaces is annotated with the specific metric decision its answer would resolve. In our setting, the missing information is not needed to answer a user query, but to determine whether a candidate metric is appropriate, what it should measure, and how it should be interpreted. We distinguish this from mainstream question generation (QG/NQG), which generates answerable comprehension questions from a passage \citep{mulla2023automatic,ijcai2024p889,Flor2025}---a different speech act from resolving evaluation-intent ambiguity.

Recent systems automate parts of evaluation construction. \emph{AutoMetrics} takes outputs plus labels and fits a single runnable holistic judge -- a metric implementation system \citep{ryan2026autometrics}. \emph{DynamicRubric} \citep{wang2026generatingrefiningdynamicevaluation} has the judge generate its own rubric and then score, in per-instance (\textsc{-Inst}) and dataset-wide (\textsc{-DS}) variants, with a DPO-fine-tuned generator variant (\textsc{-FineTuned}). \emph{AutoRubric} \citep{jia-etal-2026-autorubric} builds a rubric for a given target, and \emph{EvalAssist} \citep{desmond2025evalassist} helps a user interactively refine judging criteria once chosen. All assume the evaluation target is given and focus on the final output. Litmus instead elicits the target first and designs a per-stage portfolio from source code with no labels, determining what to measure, where, and why before any judge is written. This distinction is especially important for production AI pipelines, where failures may arise in intermediate components and where monitoring requires metrics that are traceable to system behavior.
...
\paragraph{Scientific QA} We use the established PaperQA2/LitQA2 \citep{skarlinski2024languageagentsachievesuperhuman} question-answering setting and evaluate the answer-synthesis stage on a key-passage corpus, isolating answer quality given relevant evidence.
...
\hyperref[sec:limitations]{Limitations}). B3 is the stitched single-vector score vs.\ the
...
Litmus emits per-stage portfolios (14 metrics for account grouping, 13 for scientific QA, 12 for inherent risk assessment) that include monitoring metrics, scope-matched LLM-as-judge metrics, and code-evaluated checks. The baselines (Section~\ref{sec:related}) instead score the final output and, unlike Litmus, produce no per-stage monitoring metrics; AutoMetrics is additionally trained on a separate client/train split, so its validity is out-of-distribution.

\subsection{Evaluation Axes}
\label{sec:evaluation_axes}

We score every system under a single protocol applied uniformly across all three domains, computing each axis identically for all systems, including ours, so no axis advantages a design by construction. We adopt two measures from the measurement-validity framework of \citet{ryan2026autometrics}: \emph{validity}, their criterion validity, measuring agreement between system scores and independent per-row quality labels; and \emph{degradation sensitivity}, their robustness sensitivity measure, testing whether a metric penalizes synthetically degraded outputs. We also report three portfolio-level \emph{design-quality} axes---\emph{coverage}, \emph{grounding}, and \emph{redundancy}---that characterize \emph{what} a metric-design system emits. Because final-output baselines were not built to optimize these axes, we treat them as descriptive diagnostics rather than a like-for-like contest. All LLM-based scoring (metric scores, the P1/P3 proxy labels, and the design-quality axes) uses temperature 0 with up to three retries and the same judge-model family. Section~\ref{sec:crossfamily} re-scores both the validity axis and the design-quality axes under two further families. Each axis is defined below; extended axis definitions and the five-level LLM-as-judge anchors are in Appendix~\ref{app:evaluation_axes}.

\paragraph{Validity} This is the \emph{criterion validity} of \citet{ryan2026autometrics}: the rank correlation (Kendall's $\tau$, Spearman's $\rho$) between a system's score on \emph{uncorrupted} (real, un-degraded) outputs and an independent \emph{per-row quality label}, one quality judgment per input--output row, used only as the external target. A \emph{scope-matched judge} is compared against the label for the stage it applies to (a retrieval judge against a retrieval-quality label, not a whole-pipeline one); for aggregate reporting we stitch a per-row score (Section~\ref{sec:results}).

\paragraph{Scoring judge and validity label} The \emph{scoring judge} applies a proposed rubric to an output and produces the score under evaluation; the \emph{validity label} is the separate target that score is compared against. On P2 the label is external LitQA2 gold \citep{skarlinski2024languageagentsachievesuperhuman}; on P1 and P3 it is an LLM-derived proxy scoped by the pipelines' routing labels and practitioner-confirmed component boundaries. Litmus uses none of these labels at design time; only the label-fitted AutoMetrics consumes them.

\paragraph{Design-quality and robustness axes} \emph{Coverage} is the fraction of a fixed, domain-specific list of reference failure concerns (ten for account grouping, six each for the others) addressed by at least one metric. \emph{Grounding} scores, on $[0,1]$, how concretely a metric refers to implementation artifacts (named modules, fields, rules, branches) rather than generic language. \emph{Redundancy} is the fraction of metric pairs judged to measure substantially the same behavior (lower is better); it is meaningful only for multi-metric portfolios, so the single-judge DynamicRubric variants are trivially non-redundant. \emph{Degradation sensitivity} is AutoMetrics' robustness check: whether a metric assigns lower scores to synthetically degraded outputs (10 rows over five severity levels per domain).

\section{Results}
\label{sec:results}

Because every Litmus metric is conditioned on the recovered component graph, we check that graph rather than assume it. On scientific QA, Litmus recovers a 30-node graph that is 65\% edge-verified against the tree-sitter call graph, with no phantom modules and an adversarial-critic confidence of 0.78 (Table~\ref{tab:archfidelity-pqa}, Appendix~\ref{app:arch}). 

On this footing, under a zero-label design-time constraint Litmus has the most favorable validity ranking: first on all three pipelines ($\rho=0.51, 0.72, 0.32$), ahead of every baseline including the label-fitted AutoMetrics, clearest on scientific QA and within overlapping intervals of the strongest baseline on P1 and P3 (Table~\ref{tab:main}). The stitched single-vector statistic keeps this order, with Litmus first and individually significant throughout ($p<0.001$). Uncapped coverage is top or tied on every pipeline, but that lead survives a size-matched comparison only on account grouping (Section~\ref{sec:sizecontrolled}). Litmus uniquely produces per-stage and operational metrics rather than scoring the final output alone; its \emph{redundancy} is near zero throughout, well below AutoMetrics; and on \emph{grounding} it leads on P1 and P3, trailing only slightly on P2. Litmus's judges are also among the most sensitive to corrupted outputs---top or tied on degradation on P1 and P3, and on par with the rubric systems on P2 (0.94 vs.\ 0.95)---so it is the only portfolio that is strong on both validity and degradation sensitivity across all three pipelines.

\section{Robustness}
\label{sec:robustness}

\subsection{Cross-Family Judge Control}
\label{sec:crossfamily}
To rule out a system being graded by a relative of itself, we re-score the held-out P2 rows---the one pipeline with external gold (LitQA2, $n{=}58$)---with OpenAI GPT-5.5 and GPT-4o, holding metrics, outputs, and gold labels fixed. The control covers every LLM-judge system; AutoMetrics is code-based and judge-invariant. Litmus stays first, its Spearman correlation to LitQA2 gold rising from $0.72$ under Claude to $0.86$ under either OpenAI judge (Table~\ref{tab:crossfamily}). The DynamicRubric baselines swing sharply with the assessor---the two rubric-only variants move from slightly negative under Claude to $+0.56$--$0.75$ under OpenAI, and the fine-tuned variant from $0.27$ to $0.48$--$0.56$---so their ordering is largely an artifact of the judge. We do \emph{not} claim judge-invariance on the proxy-labeled P1/P3.

The same worry applies to coverage, grounding, and redundancy. Re-scoring every portfolio under GPT-4o and GPT-5.5 leaves the ordering intact: Litmus stays ahead of AutoMetrics on grounding and redundancy in all nine pipeline--judge cells, and is first or tied on coverage in eight of nine (Table~\ref{tab:crossfamilyaxes}, Appendix~\ref{app:axescontrol}). Absolute scores shift with the assessor; the ranking does not.
...
\begin{tabular}{lccc}
\toprule
System & Claude & GPT-5.5 & GPT-4o \\
\midrule
AutoMetrics & 0.69 & 0.69 & 0.69 \\
DynRub-\textsc{Inst} & $-0.06$ & 0.75 & 0.56 \\
DynRub-\textsc{DS} & $-0.12$ & 0.67 & 0.69 \\
DynRub-\textsc{FT} & 0.27 & 0.48 & 0.56 \\
\textbf{Litmus} & \textbf{0.72} & \textbf{0.86} & \textbf{0.86} \\
\bottomrule
\end{tabular}
\caption{Cross-family judge control on scientific QA (P2, $n{=}58$): validity (Spearman $\rho$ vs.\ LitQA2 gold) under three judge families. Litmus stays first under two disjoint OpenAI judges while the rubric baselines swing with the judge; AutoMetrics is code-based and judge-invariant. Per-row Claude--OpenAI agreement: $\rho=0.64$, $\tau=0.63$, $\kappa\approx0.5$.}
\label{tab:crossfamily}
\end{table}

\subsection{Size-Controlled Coverage}
\label{sec:sizecontrolled}
Coverage grows with portfolio size, and Litmus emits more metrics than the baselines, so part of its coverage lead could be mechanical. We cap Litmus at AutoMetrics' eight metrics and recompute coverage two ways: a critic-verdict-ranked top-$k$ (pass $>$ flag $>$ unscored $>$ reject) and the mean over six random size-eight subsets (Table~\ref{tab:sizecontrolled}). On P1 the gap remains: $0.70$ ranked / $0.65\pm0.08$ random versus AutoMetrics' $0.30$. On P2 the full-size $1.00$ falls to a tie at $0.67$, and P3 is a tie at every size ($0.83$). Only P1 keeps a coverage lead at matched size.

\subsection{Adversarial-Validity Gate Ablation}
\label{sec:gateablation}
On the P2 draft the gate rejects $11$ of $77$ metrics and flags $10$. Dropping the rejects leaves coverage unchanged ($1.00$), cuts redundancy by $35\%$ ($0.024\rightarrow0.016$), and raises grounding ($0.377\rightarrow0.393$; Table~\ref{tab:gate}). The rejects are pipeline-mismatched (retrieval metrics on a PDF reader that does no query-time retrieval); the one scoreable metric is untouched ($\rho=0.72$).

\subsection{Reconstruction Fidelity vs.\ Metric Validity}
\label{sec:fidelityvalidity}
We pair each scoreable metric's validity with the host subsystem's reconstruction fidelity (Table~\ref{tab:fidelityvalidity}). Aggregate fidelity does not predict validity: P1 has higher edge-verification than P2 ($83\%$ vs.\ $65\%$) yet lower headline validity, and the pooled correlation is near zero (Spearman $-0.06$, $n{=}7$; component-level $-0.50$, $n{=}3$---neither powered). The gate still drops metrics whose reconstructed responsibility cannot support them (Section~\ref{sec:gateablation}). The lowest-confidence P2 node (\texttt{settings-config}, $0.65$) is exactly where the critic flagged the metric scoped there. With only $1$--$4$ scoreable metrics per pipeline a powered slope is not estimable.

These checks support a branch-diversity hypothesis, not an established predictor: elicitation helps most when a pipeline has multiple decision paths whose intended behavior is not recoverable from code---abstention semantics, fallback behavior, and a recall-versus-precision preference are the recurring cases on P2---and least when one dominant branch carries the signal (P1 and P3), where the validity intervals overlap the strongest baseline.

\section{Conclusion and Future Work}
\label{sec:conclusion}

We reframed automatic metric design as goal elicitation by interrogation: rather than selecting metrics first, Litmus interrogates a pipeline's source code and its practitioner to make the evaluation target explicit, then admits, rejects, or re-scopes each candidate against the resolved constraints. Across three domains this zero-label procedure yields near-zero redundancy and ranks first on label validity, clearest on scientific QA, with a coverage advantage on account grouping (Section~\ref{sec:sizecontrolled}); both that ranking on scientific QA and the design-quality picture on all three hold under a different judge family (Section~\ref{sec:robustness}). Key next steps are a controlled human study of portfolios generated with and without practitioner interrogation on the same pipeline, then independently rated and repaired, with rating deltas and edit distances as the outcomes; a full regeneration ablation of the four un-ablated stages (\hyperref[sec:limitations]{Limitations}); and broader pipelines.

\section*{Limitations}
\label{sec:limitations}

First, scope. Our \textbf{empirical evaluation} spans three real, code-defined pipelines from different domains---account grouping, scientific QA, and inherent risk assessment---primarily scored by a single judge-model family. We ran a cross-family judge control on the one pipeline with external gold labels (P2), where Litmus's validity ranking holds under two disjoint OpenAI judges (Section~\ref{sec:robustness}); on the proxy-labeled pipelines (P1, P3), whose validity labels are LLM-derived, judge-system alignment is not ruled out, so those results should still be read as evidence within one assessor family. Litmus itself is pipeline-agnostic, with no assumptions specific to any one domain, and broader multi-judge evaluation remains future work. Second, the systems are not strictly commensurable: Litmus is a zero-label metric-design system, whereas AutoMetrics is label-fitted and DynamicRubric is rubric-generating, and both score only the final output. AutoMetrics is fit on a separate client dataset and evaluated on the held-out validity rows, so its reported figure is already an out-of-distribution estimate rather than an optimistic in-distribution one. We treat this role difference as intentional rather than a confound, but it means absolute numbers should be read as characterizing what each system produces; the defensible claim is the ranking under a shared zero-label, design-time constraint. Third, three of our axes (coverage, grounding, and redundancy) are themselves LLM-assessed, and primarily by the model family that powers Litmus, risking judge--system alignment bias \citep{zheng2023judging,panickssery2024llm}; we mitigate this with a broad, fixed set of reference concerns per pipeline, and re-scoring every portfolio on all three pipelines under two further judge families leaves the ordering against AutoMetrics intact on grounding and redundancy in every pipeline--judge cell and on coverage in eight of nine (Section~\ref{sec:crossfamily}, Appendix~\ref{app:axescontrol}). Absolute scores still inherit the assessor's biases---one judge raises a baseline's account-grouping coverage from $0.30$ to $0.80$---so the ranking is again the more defensible claim. Fourth, coverage is monotone in portfolio size---Litmus emits a substantially larger portfolio than the baselines (14 metrics versus 1--8 on account grouping: AutoMetrics emits 8 and DynamicRubric a single holistic rubric score)---and the reference concern lists were authored by us. A size-controlled analysis (Section~\ref{sec:sizecontrolled}) shows the advantage survives capping only on P1. Reconstruction fidelity does not predict downstream validity in this sample (Section~\ref{sec:fidelityvalidity}), and only one of the five design stages is ablated: Section~\ref{sec:gateablation} isolates the adversarial-validity gate, while reconstruction, graph verification, clarification, and downstream metric design remain un-ablated, since a toggle-based ablation would require regenerating both arms of a non-deterministic synthesis step. Finally, the clarification channel's end-to-end benefit is shown here only at the specification level: we do not show that an answer changes which metrics enter the portfolio, or that validity improves as a result. Operational use does not establish improvement over prior manual or heuristic metric design; the controlled human study that would establish it, and a longitudinal evaluation of whether re-running Litmus on changed code keeps a portfolio current, both remain future work.
...
\label{app:concerns}
The coverage axis uses a fixed list of reference failure-concerns per pipeline. For account grouping (ten): (1) account assigned to a semantically wrong group; (2) group inconsistent with / contradicting the stated FSLI; (3) business process context ignored; (4) high confidence on an incorrect assignment (miscalibration); (5) missing or empty reasoning; (6) grouping not grounded in retrieved evidence (hallucination); (7) wrong granularity of disaggregation; (8) override rules for account types not respected; (9) operational failure (exceptions/retries/degraded service); (10) retrieval quality issues (low recall / irrelevant chunks). For scientific QA (six): (1) chosen option does not match the correct answer; (2) answer not grounded in the retrieved evidence (hallucinated); (3) answer contradicts the cited passage; (4) relevant evidence present but ignored / not used; (5) overconfident answer when the evidence is insufficient; (6) incomplete answer that omits key parts of the question. For inherent risk assessment (six): (1) wrong risk severity level assigned; (2) missing or wrong risk category; (3) unsupported / hallucinated risk not grounded in input; (4) missing or low-quality mitigation recommendation; (5) inconsistent or contradictory risk justification; (6) relevant context not retrieved / used.

\section{Scoring and Implementation Details}
\label{sec:scoring_details}
The LLM scoring protocol (temperature 0, up to three retries, and one judge family for metric scores, P1/P3 proxy labels, and design-quality axes) is stated in Section~\ref{sec:evaluation_axes}. The five-level anchors used by Litmus's LLM-as-judge metrics are in Appendix~\ref{app:evaluation_axes}; the designer prompt that embeds them is in Appendix~\ref{app:impl}. Judge-family alignment bias is discussed in \hyperref[sec:limitations]{Limitations} and controlled for in Section~\ref{sec:crossfamily}; Appendix~\ref{app:axescontrol} reports the design-quality axes under three judge families.

The coverage reference lists contain ten concerns for account grouping and six each for scientific QA and inherent risk assessment (enumerated in Appendix~\ref{app:concerns}). For degradation sensitivity, each domain corrupts 10 rows across five cumulative severity levels.
...
\paragraph{LLM-as-judge scoring scale.}
Litmus's LLM-as-judge metrics use one five-level scale. Every score is accompanied by a chain-of-thought rationale: the judge must explain its reasoning, consider both strengths and weaknesses, and then assign a score. The anchors, taken from the designer prompt in Appendix~\ref{app:impl}, are:
\begin{itemize}
  \item 5 (Excellent): Fully meets all criteria with no issues.
  \item 4 (Good): Meets criteria with only minor issues.
  \item 3 (Acceptable): Partially meets criteria with some issues.
  \item 2 (Poor): Largely fails to meet criteria with major issues.
  \item 1 (Very Poor): Fails to meet criteria entirely.
\end{itemize}

\subsection{Cross-Family Control on the Design-Quality Axes}
\label{app:axescontrol}

Coverage, grounding, and redundancy are scored from metric names and descriptions alone, by the same notebook functions used for Table~\ref{tab:main}. We held every portfolio fixed and re-ran those functions under Claude Opus~4.6 (the family that also powers Litmus), GPT-4o, and GPT-5.5, one execution per pipeline so the three columns of a pipeline are comparable. Across the two runs the three families issued $1{,}503$ judge calls; every response parsed, with no empty or off-format replies.

The Claude column is that re-run, not a copy of Table~\ref{tab:main}. Redundancy reproduced exactly on all three pipelines; grounding to within $0.008$. Coverage moved by one flipped concern on account grouping ($0.80\rightarrow 0.90$) and on scientific QA ($1.00\rightarrow 0.83$), and was exact on risk assessment. Coverage is ten binary items on the first pipeline and six on the other two, so a single flip is the smallest possible disagreement; judges are called at temperature 0 but are not bit-deterministic. Mixing the published Claude numbers with a fresh OpenAI column would put two executions of a non-deterministic judge side by side, so we do not.
...
\toprule
& \multicolumn{3}{c}{P1 Account Grouping} & \multicolumn{3}{c}{P2 PaperQA2/LitQA2} & \multicolumn{3}{c}{P3 Risk Assessment} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}\cmidrule(lr){8-10}
System & Claude & GPT-4o & GPT-5.5 & Claude & GPT-4o & GPT-5.5 & Claude & GPT-4o & GPT-5.5 \\
\midrule
\multicolumn{10}{l}{\emph{Coverage} $\uparrow$} \\
AutoMetrics            & 0.300 & 0.800 & 0.300 & 0.833 & 1.000 & 1.000 & 0.833 & 0.833 & 0.833 \\
DynRub-\textsc{Inst}   & 0.500 & 0.300 & 0.400 & 0.667 & 1.000 & 0.667 & 0.667 & 0.833 & 0.667 \\
DynRub-\textsc{DS}     & 0.400 & 0.300 & 0.300 & 0.667 & 0.833 & 0.333 & 0.000 & 0.500 & 0.500 \\
DynRub-\textsc{FT}     & 0.500 & 0.800 & 0.200 & 0.833 & 1.000 & 0.500 & 0.667 & 1.000 & 0.500 \\
\textbf{Litmus}        & \textbf{0.900} & \textbf{1.000} & \textbf{0.900} & \textbf{0.833} & \textbf{1.000} & \textbf{0.833} & \textbf{0.833} & \textbf{1.000} & \textbf{0.833} \\
\midrule
\multicolumn{10}{l}{\emph{Grounding} $\uparrow$} \\
AutoMetrics            & 0.394 & 0.375 & 0.337 & 0.175 & 0.212 & 0.138 & 0.163 & 0.037 & 0.069 \\
DynRub-\textsc{Inst}   & 0.300$^{\dagger}$ & 0.000$^{\dagger}$ & 0.250$^{\dagger}$ & 0.150$^{\dagger}$ & 0.000$^{\dagger}$ & 0.350$^{\dagger}$ & 0.150$^{\dagger}$ & 0.000$^{\dagger}$ & 0.300$^{\dagger}$ \\
DynRub-\textsc{DS}     & 0.350$^{\dagger}$ & 0.300$^{\dagger}$ & 0.350$^{\dagger}$ & 0.350$^{\dagger}$ & 0.200$^{\dagger}$ & 0.350$^{\dagger}$ & 0.350$^{\dagger}$ & 0.300$^{\dagger}$ & 0.350$^{\dagger}$ \\
DynRub-\textsc{FT}     & 0.350$^{\dagger}$ & 0.700$^{\dagger}$ & 0.450$^{\dagger}$ & 0.250$^{\dagger}$ & 0.300$^{\dagger}$ & 0.350$^{\dagger}$ & 0.250$^{\dagger}$ & 0.300$^{\dagger}$ & 0.450$^{\dagger}$ \\
\textbf{Litmus}        & \textbf{0.450} & \textbf{0.536} & \textbf{0.399} & \textbf{0.331} & \textbf{0.331} & \textbf{0.269} & \textbf{0.417} & \textbf{0.375} & \textbf{0.323} \\
\midrule
\multicolumn{10}{l}{\emph{Redundancy} $\downarrow$} \\
AutoMetrics            & 0.143 & 0.179 & 0.143 & 0.179 & 0.179 & 0.107 & 0.071 & 0.036 & 0.036 \\
DynRub-\textsc{Inst}   & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ \\
DynRub-\textsc{DS}     & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ \\
DynRub-\textsc{FT}     & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ & 0.000$^{\ddagger}$ \\
\textbf{Litmus}        & \textbf{0.022} & \textbf{0.011} & \textbf{0.011} & \textbf{0.013} & \textbf{0.000} & \textbf{0.000} & \textbf{0.045} & \textbf{0.000} & \textbf{0.000} \\
\bottomrule
\end{tabular}
\caption{Cross-family judge control on the three LLM-assessed design axes, all three pipelines. Portfolios are held fixed and re-scored by Claude Opus~4.6, GPT-4o, and GPT-5.5 in a single execution per pipeline. Against AutoMetrics, the only other multi-metric portfolio, Litmus is better on grounding and on redundancy in all nine pipeline--judge cells, and first or tied on coverage in eight of nine; the exception is scientific QA under GPT-5.5. Absolute levels shift with the assessor; the ordering does not. $\dagger$: single-metric portfolio, so grounding is the mean of one judge call. $\ddagger$: redundancy is defined over unordered metric pairs, so a one-metric portfolio scores $0.000$ by construction. Litmus portfolio sizes: 14 / 13 / 12; AutoMetrics 8 and each DynamicRubric variant 1 on every pipeline.}
\label{tab:crossfamilyaxes}
\end{table*}

\subsection{Size Control, Gate Ablation, and Reconstruction Fidelity}
\label{app:robustness-tables}

Tables for the size-controlled coverage check, the adversarial-validity gate ablation, and the reconstruction-fidelity pairing (Sections~\ref{sec:sizecontrolled}--\ref{sec:fidelityvalidity}).
...
\begin{tabular}{lcccc}
\toprule
Pipeline & Full & @8 rank & @8 rand & Auto. \\
\midrule
P1 Acct.\ grouping & 0.80 & 0.70 & $0.65{\pm}.08$ & 0.30 \\
P2 Scientific QA & 1.00 & 0.67 & $0.58{\pm}.32$ & 0.67 \\
P3 Risk assessment & 0.83 & 0.83 & $0.83{\pm}.00$ & 0.83 \\
\bottomrule
\end{tabular}
\caption{Size-controlled coverage. Litmus full portfolio vs.\ capped to $k{=}8$ (AutoMetrics' size) by verdict-ranking and by random size-8 subsets (mean$\pm$sd), vs.\ AutoMetrics. The coverage advantage survives capping only on P1; P2 and P3 tie at matched size.}
\label{tab:sizecontrolled}
\end{table}
...
\begin{tabular}{lccc}
\toprule
Axis & Ungated (77) & Gated (66) & $\Delta$ \\
\midrule
Coverage $\uparrow$ & 1.000 & 1.000 & $+0.000$ \\
Redundancy $\downarrow$ & 0.024 & 0.016 & $-35\%$ \\
Grounding $\uparrow$ & 0.377 & 0.393 & $+0.017$ \\
\bottomrule
\end{tabular}
\caption{Adversarial-validity gate ablation on scientific QA (P2). Counts are over the whole-solution drafted portfolio across all components, not the 13-metric scoped portfolio in Table~\ref{tab:main}. Removing the $11$ rejected metrics (of $77$ drafted; $10$ flagged) cuts redundancy by $35\%$ and raises grounding at no coverage cost; the one scoreable metric is unchanged ($\rho=0.72$).}
\label{tab:gate}
\end{table}
...
\begin{tabular}{lcccc}
\toprule
Pipeline & Edge-ver.\% & Conf.\ & Min & Validity $\rho$ \\
\midrule
P1 Acct.\ grouping & 83 & 0.93 & 0.90 & 0.51 \\
P2 Scientific QA & 65 & 0.83 & 0.65 & 0.72 \\
P3 Risk assessment & 83 & 0.92 & 0.88 & 0.32 \\
\bottomrule
\end{tabular}
\caption{Reconstruction fidelity vs.\ downstream validity. Edge-verification \% and host-subsystem critic confidence (mean, min) against headline validity $\rho$. P2's $0.83$ is the five-node \texttt{core-qa-engine} mean; whole-graph confidence across all 30 nodes is $0.78$ (Section~\ref{sec:results}). Aggregate fidelity does not track validity (pooled $\rho=-0.06$, $n{=}7$).}
\label{tab:fidelityvalidity}
\end{table}
...
\item \textbf{Data assumptions.} ``Can \emph{RAG Retrieval Recall@K} on the retrieval stage be computed?''
...
\item \textbf{Answer Generation \& Formatting:} ``Should an answer that abstains (``insufficient evidence'') count as a success or a failure?'' \emph{Sets the direction of goodness for the abstention/refusal-rate metric: counted as success, the rate should be high; counted as failure, low.}
...
\bottomrule
\end{tabular}
\caption{Architecture-reconstruction fidelity for the scientific-QA pipeline (PaperQA2). The seven subsystems are PDF reader plugins, agent orchestration, metadata clients, the core QA engine, the Nemotron reader, contributed sources, and tests. Strict per-module F1 (0.64) is lower than coverage F1 because Litmus clusters the 32 source modules into 30 functional nodes as shown in Figure \ref{fig:archpqa} rather than emitting one node per file. All values are recorded from the persisted graph and critic annotations (Appendix~\ref{app:arch}).}
\label{tab:archfidelity-pqa}
\end{table}

\paragraph{Fidelity vs.\ validity pairing.}
The $n{=}7$ pooled points in Section~\ref{sec:fidelityvalidity} are the scoreable metrics (non-null criterion validity) across the three pipelines, each paired with its host-subsystem mean critic confidence. Component-level $n{=}3$ is one point per pipeline. For P2 the host is \texttt{core-qa-engine} (five nodes, mean $0.83$); the $0.78$ above is the mean over all 30 nodes.

\paragraph{How the numbers were recorded.}
The graph and critic annotations are persisted per solution in the system's own store, so these figures are recomputable in a deployment without new model calls; the client artifacts themselves are proprietary and are not released (\hyperref[sec:ethics]{Ethical Considerations}). The queries below are the recording path, not a public reproduction recipe:
{\footnotesize
\begin{verbatim}
