...
To rule out a system being graded by a relative of itself, we re-score the held-out P2 rows---the one pipeline with external gold (LitQA2, $n{=}58$)---with scoring judges from a \emph{different} family (OpenAI GPT-5.5 and, separately, GPT-4o) than the Claude family that powers Litmus, holding the generated metrics, evaluated outputs, and gold labels fixed. The control is applied to \emph{every} LLM-judge system; AutoMetrics is code-based and thus judge-invariant. Litmus stays first and if anything strengthens, its Spearman correlation to the LitQA2 gold rising from $0.72$ under Claude to $0.86$ under either OpenAI judge (Table~\ref{tab:crossfamily}). The DynamicRubric baselines are more judge-sensitive, swinging from slightly negative under Claude to $+0.56$--$0.75$ under the OpenAI judges, so their apparent ordering is largely an artifact of the assessor. Claude and OpenAI scores agree at Spearman $\rho=0.64$, Kendall $\tau=0.63$, Cohen's $\kappa\approx0.5$ at the median split. We scope this control to P2 because it is the only pipeline with external gold, and we do \emph{not} claim judge-invariance on the proxy-labeled P1/P3.

The same worry applies to coverage, grounding, and redundancy, which are LLM-assessed by the family that also powers Litmus. We re-scored all three for every portfolio on all three pipelines under GPT-4o and GPT-5.5, holding the portfolios fixed and running each pipeline's three arms in a single execution (Appendix~\ref{app:axescontrol}). What moves is the calibration, not the ordering: GPT-4o is markedly more generous, lifting AutoMetrics' account-grouping coverage from $0.30$ to $0.80$, yet Litmus stays ahead of AutoMetrics---the only other multi-metric portfolio---on grounding and redundancy in all nine pipeline--judge cells, and is first or tied on coverage in eight of nine. The exception is scientific QA under GPT-5.5, where AutoMetrics addresses all six reference concerns and Litmus five. We read this as rank stability under a change of assessor, not score invariance.
...
We isolate the adversarial-validity gate on the P2 draft. Of $77$ drafted metrics it rejects $11$ and flags $10$. Removing the rejects leaves coverage unchanged ($1.00$), cuts redundancy by $35\%$ ($0.024\rightarrow0.016$), and raises grounding ($0.377\rightarrow0.393$; Table~\ref{tab:gate}). The rejects are near-duplicate, pipeline-mismatched metrics---\emph{RAG Context Precision} and \emph{RAG Retrieval Recall@K} on the PDF reader, which does no query-time retrieval---and the one scoreable metric is untouched ($\rho=0.72$). This is a single-stage ablation of the already-critiqued portfolio; a full regeneration ablation, and ablations of reconstruction, graph verification, clarification, and downstream metric design, remain future work.
...
We pair each scoreable metric's criterion validity with the host subsystem's reconstruction fidelity (mean per-node critic confidence; Table~\ref{tab:fidelityvalidity}). Aggregate fidelity does not predict validity: P1 has higher edge-verification than P2 ($83\%$ vs.\ $65\%$) yet lower headline validity, and the pooled correlation is near zero (Spearman $-0.06$, $n{=}7$; component-level $-0.50$, $n{=}3$---neither powered). The gate still rejects metrics whose reconstructed responsibility does not support them (retrieval metrics on the PDF reader; Section~\ref{sec:gateablation}), a safeguard against some reconstruction errors. The lowest-confidence P2 node (\texttt{settings-config}, $0.65$) is exactly where the critic flagged the metric scoped there. With only $1$--$4$ scoreable metrics per pipeline we cannot estimate a powered slope; a powered per-component study is left to future work.
...
The LLM scoring protocol (temperature 0, up to three retries, and one judge family for metric scores, P1/P3 proxy labels, and design-quality axes) is stated in Section~\ref{sec:evaluation_axes}. Judge-family alignment bias is discussed in Section~\ref{sec:limitations} and controlled for in Section~\ref{sec:crossfamily}; Appendix~\ref{app:axescontrol} reports the design-quality axes under three judge families.
