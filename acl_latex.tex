...
\paragraph{Validity} This is the \emph{criterion validity} of \citet{ryan2026autometrics}: the association (Kendall's $\tau$, Spearman's $\rho$) between a system's score on uncorrupted outputs and an independent per-row quality label. Scope-matched judges are scored against the matching label; for aggregate reporting we compute a stitched per-row score (Section~\ref{sec:results}).
