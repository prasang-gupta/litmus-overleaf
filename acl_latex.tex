...
We score every system under one protocol applied uniformly across all three domains, computing each axis identically for all systems including ours -- so no axis advantages a design by construction. We adopt two measures from the measurement-validity framework of \citet{autometrics}. Our \emph{validity} is their \emph{criterion validity}: agreement between a system's scores and an independent per-row quality label. \emph{Degradation sensitivity} is their robustness \emph{sensitivity} measure: whether a metric penalizes synthetically degraded outputs. We additionally report three portfolio-level \emph{design-quality} axes---\emph{coverage}, \emph{grounding}, and \emph{redundancy}---that characterize \emph{what} a metric-design system emits; since the final-output baselines were not built to optimize these, we treat them as descriptive diagnostics rather than a like-for-like contest. Each axis is defined in its subsection below; full scoring rubrics are in Appendix~\ref{app:arch}.
