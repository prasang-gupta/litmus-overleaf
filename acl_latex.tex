...
validity statistic, applied uniformly across the three pipelines (P1: account
grouping; P2: scientific QA; P3: inherent risk). The pattern is consistent:
Litmus is the broadest and least redundant portfolio with the strongest label
validity. It is top or tied on \emph{coverage} across all pipelines and uniquely
produces per-stage and operational metrics rather than scoring the final output
alone; its \emph{redundancy} is near zero throughout, well below AutoMetrics; and
on \emph{grounding} it leads on P1 and P3, trailing only slightly on P2
...
single-vector statistic preserves this ranking, and bootstrap CIs
(Table~\ref{tab:b3}) place Litmus first and individually significant throughout
($p<0.001$), though on P1 and P3 its interval overlaps the strongest baseline, so
we claim a favorable ranking under the zero-label constraint rather than a
uniformly significant margin. Litmus's judges are also among the most sensitive
to corrupted outputs (tied with the rubric systems on P2), reacting to
degradation at least as strongly as every baseline---making it the only portfolio
strong on both validity and degradation sensitivity across pipelines.
