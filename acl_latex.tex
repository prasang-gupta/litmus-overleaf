...
\paragraph{Scientific QA} We use the established PaperQA2/LitQA2 \cite{skarlinski2024language} question-answering setting and evaluate the answer-synthesis stage on a key-passage corpus, isolating answer quality given relevant evidence.
...
These checks are consistent with a branch-diversity reading: elicitation helps most when a pipeline has multiple decision paths whose intended behavior is not recoverable from code---abstention semantics, fallback behavior, and a recall-versus-precision preference are the recurring cases on P2---and least when one dominant branch carries the signal (P1 and P3), where the validity intervals overlap the strongest baseline. We offer this as a hypothesis consistent with the three pipelines, not an established predictor.

\section{Conclusion and Future Work}
\label{sec:conclusion}

We reframed automatic metric design as goal elicitation by interrogation: rather than selecting metrics first, Litmus interrogates a pipeline's source code and its practitioner to make the evaluation target explicit, then admits, rejects, or re-scopes each candidate against the resolved constraints. Across three domains this zero-label procedure yields near-zero redundancy and ranks first on label validity, decisively on scientific QA, with a coverage advantage on account grouping (Section~\ref{sec:sizecontrolled}); both that ranking and the design-quality picture hold under a different judge family (Section~\ref{sec:robustness}). Key next steps are a controlled human study (portfolios generated with and without practitioner interrogation on the same pipeline, then independently rated and repaired by practitioners, with rating deltas and edit distances as the outcomes), a full regeneration ablation of the un-ablated stages (Section~\ref{sec:gateablation}), and broader pipelines.
...
The LLM scoring protocol (temperature 0, up to three retries, and one judge family for metric scores, P1/P3 proxy labels, and design-quality axes) is stated in Section~\ref{sec:evaluation_axes}, as are the five-level anchors used by Litmus's LLM-as-judge metrics; the designer prompt that embeds those anchors is in Appendix~\ref{app:impl}. Judge-family alignment bias is discussed in Section~\ref{sec:limitations} and controlled for in Section~\ref{sec:crossfamily}; Appendix~\ref{app:axescontrol} reports the design-quality axes under three judge families.
